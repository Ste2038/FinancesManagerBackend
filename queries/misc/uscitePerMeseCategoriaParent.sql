SELECT C.nome AS 'nome', SUM(T.importo) AS 'importo', MONTH(T.DATETIME) AS 'mese', C.idCategoriaParent
FROM Transazioni AS T, Categorie AS C
WHERE T.idCategoria = C.idCategoria 
AND YEAR(T.DATETIME) = ${anno} AND T.idCategoria IS NOT NULL AND C.isUscita = 1 AND C.isEntrata = 0
